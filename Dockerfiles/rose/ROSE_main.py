...
def main():
    ...
    cwd = os.getcwd()
    genomeDict = {
        'HG18':'%s/annotation/hg18_refseq.ucsc' % dir,
        'MM9': '%s/annotation/mm9_refseq.ucsc' % dir,
        'HG19':'%s/annotation/hg19_refseq.ucsc' % dir,
        'HG38':'%s/annotation/hg38_refseq.ucsc' % dir,
        'MM8': '%s/annotation/mm8_refseq.ucsc' % dir,
        'MM10':'%s/annotation/mm10_refseq.ucsc' % dir,
        }
    ...
    for bamFile in bamFileList:
        ...
        cmd1 = "%s/ROSE_bamToGFF.py -f 1 -e 200 -r -m %s -b %s -i %s -o %s &" % (dir,nBin,bamFile,stitchedGFFFile,mappedOut1)
        print(cmd1)
        os.system(cmd1)
        ...
        #WILL TRY TO RUN AS A BACKGROUND PROCESS. BATCH SUBMIT THIS LINE TO IMPROVE SPEED
        cmd2 = "%s/ROSE_bamToGFF.py -f 1 -e 200 -r -m %s -b %s -i %s -o %s &" % (dir,nBin,bamFile,inputGFFFile,mappedOut2)
        print(cmd2)
        os.system(cmd2)
    ...
    if options.control:
        ...
        cmd = 'R --no-save %s %s %s %s < %s/ROSE_callSuper.R' % (outFolder,outputFile1,inputName,controlName,dir)
