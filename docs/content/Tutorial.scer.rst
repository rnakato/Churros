...
.. code-block:: bash
    ...
    churros --preset scer -p 12 samplelist.txt samplepairlist.txt sacCer3 Ensembl-R64-1-1/

``-p 12`` specifies the number of CPUs. ``sacCer3`` is the UCSC genome build and ``Ensembl-R64-1-1`` is the directory generated by ``download_genomedata.sh`` and ``build-index.sh``.
...
.. code-block:: bash
    ...
    churros_visualize samplepairlist.txt drompa+ sacCer3 Ensembl-R64-1-1/ --preset scer --enrich
